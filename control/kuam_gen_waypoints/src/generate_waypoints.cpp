#include "kuam_gen_waypoints/generate_waypoints.h"
...
bool GenerateWaypoints::GetParam()
{
    ...
    m_nh.getParam(node_name_with_namespace + "/x_offset_m", m_x_offset_m_param);
    m_nh.getParam(node_name_with_namespace + "/z_offset_m", m_z_offset_m_param);
    m_nh.getParam(node_name_with_namespace + "/ugv_name", m_vehicle_name_param);
    m_nh.getParam(node_name_with_namespace + "/distance_threshold", m_distance_thresh_param);
    m_nh.getParam(node_name_with_namespace + "/target_wp_pub_interval", m_target_wp_pub_interval_param);
    m_nh.getParam(node_name_with_namespace + "/detected_dead_band", m_detected_dead_band_param);
    m_nh.getParam(node_name_with_namespace + "/global_to_local", m_global_to_local_param);    
    m_nh.getParam(node_name_with_namespace + "/alt_offset_m", m_alt_offset_m_param);
...
bool GenerateWaypoints::InitROS()
{
    ...
    m_command_sub = 
        m_nh.subscribe<uav_msgs::Chat>("/control/chat/command", 10, boost::bind(&GenerateWaypoints::ChatterCallback, this, _1));
    m_init_pose_sub = 
...
bool GenerateWaypoints::IsValid(vector<geometry_msgs::Pose> &poses, geometry_msgs::Point curr_position, bool is_waypoint)
{
...
bool GenerateWaypoints::IsValid(vector<geographic_msgs::GeoPoseStamped> &poses, geographic_msgs::GeoPoint curr_position)
{
    if (m_utils.IsNan(curr_position)){
        return false;
    }
...
bool GenerateWaypoints::IsValid(vector<geometry_msgs::Pose> &poses)
{
    if (poses.size() == 0){
...
bool GenerateWaypoints::IsValid(vector<geographic_msgs::GeoPoseStamped> &poses)
{
    if (poses.size() == 0){
        ROS_ERROR_STREAM("[generate_waypoints] Waypoints is empty");
        return false;
