#include "kuam_offb/tf_broadcaster.h"
#include <math.h>
#include <Eigen/Dense>
...
bool TfBroadcaster::GetParam()
{
    ...
    m_nh.getParam(node_name_with_namespace + "/is_finding_home", m_is_finding_home_param);
...
void TfBroadcaster::InitStaticTf(void)
{
    ...
    static tf2_ros::StaticTransformBroadcaster camera_link_static_tf_br;
    geometry_msgs::TransformStamped camera_link_tf_stamped;

    camera_link_tf_stamped.header.stamp = ros::Time::now();
    camera_link_tf_stamped.header.frame_id = "base_link";
    camera_link_tf_stamped.child_frame_id = "camera_link";

    camera_link_tf_stamped.transform.translation.x = 0.1;
    camera_link_tf_stamped.transform.translation.y = 0.0;
    camera_link_tf_stamped.transform.translation.z = 0.0;
    
    tf2::Quaternion q;
    q.setRPY(0, M_PI, 0);
    camera_link_tf_stamped.transform.rotation.x = q.x();
    camera_link_tf_stamped.transform.rotation.y = q.y();
    camera_link_tf_stamped.transform.rotation.z = q.z();
    camera_link_tf_stamped.transform.rotation.w = q.w();
    camera_link_static_tf_br.sendTransform(camera_link_tf_stamped);
}
