...
bool ArucoTracking::GetParam()
{
    ...
    m_nh.getParam(node_name_with_namespace + "/target_marker_id", m_target_marker_id_param);
    m_nh.getParam(node_name_with_namespace + "/filter_buf_size", m_filter_buf_size_param);
...
bool ArucoTracking::InitROS()
{
    ...
    m_markers_pub = m_nh.advertise<visualization_msgs::MarkerArray> (node_name_with_namespace + "/aruco_markers", 1);
    m_target_state_pub = m_nh.advertise<kuam_msgs::TargetState> (node_name_with_namespace + "/target_state", 1);
    m_target_states_pub = m_nh.advertise<kuam_msgs::TargetStateArray> (node_name_with_namespace + "/target_states", 1);
    
    // Initialize timer
    m_image_timer = m_nh.createTimer(ros::Duration(0.01), &ArucoTracking::ImageTimerCallback, this);
...
void ArucoTracking::ImageTimerCallback(const ros::TimerEvent& event)
{
    if ((m_cv_ptr != nullptr) || !m_using_image_msg_param){
        vector<int> ids;
        vector<Vec3d> rvecs, tvecs;
        MarkerPoseEstimating(ids, rvecs, tvecs);
        
        geometry_msgs::Pose target_pose;
        bool is_detected = false;
        Camera2World(ids, rvecs, tvecs, target_pose, is_detected);

        TargetStateEstimating(target_pose, is_detected);
        VisualizeTarget();
        Publish();
    }
}
...
bool ArucoTracking::MarkerPoseEstimating(vector<int>& ids, vector<Vec3d>& rvecs, vector<Vec3d>& tvecs)
{
...
bool ArucoTracking::Camera2World(const vector<int> ids, const vector<Vec3d> rvecs, const vector<Vec3d> tvecs, geometry_msgs::Pose& target_pose, bool& is_detected)
{
...
bool ArucoTracking::TargetStateEstimating(const geometry_msgs::Pose pose, bool& is_detected)
{
    Eigen::Vector3d pos;
    ...
    kuam_msgs::TargetState target_msg;
    target_msg.header.frame_id = CAMERA_FRAME;
    target_msg.header.stamp = ros::Time::now();
    target_msg.id = m_target_marker_id_param;
    ...
    // kuam_msgs::TargetStateArray target_states_msg;
