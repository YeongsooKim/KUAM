...
{
    InitFlag();
    if (!GetParam()) ROS_ERROR("[setpoint_visual] Fail GetParam %s", m_err_param.c_str());
    InitROS();
...
bool Visualizer::GetParam()
{
    ...
    if (!m_nh.getParam("/data_ns", m_data_ns_param)) { m_err_param = "data_ns"; return false; }
    if (!m_nh.getParam("/maneuver_ns", m_maneuver_ns_param)) { m_err_param = "maneuver_ns"; return false; }
    if (!m_nh.getParam(m_maneuver_ns_param + "/state_machine/standby_dist_th_m", m_standby_dist_th_m_param)) { m_err_param = "standby_dist_th_m"; return false; }
    if (!m_nh.getParam(m_maneuver_ns_param + "/state_machine/landing_standby_alt_m", m_landing_standby_alt_m_param)) { m_err_param = "landing_standby_alt_m"; return false; }
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/small_marker_size_m", m_small_marker_size_m_param)) { m_err_param = "small_marker_size_m"; return false; }
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/medium_marker_size_m", m_medium_marker_size_m_param)) { m_err_param = "medium_marker_size_m"; return false; }
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/big_marker_size_m", m_big_marker_size_m_param)) { m_err_param = "big_marker_size_m"; return false; }
    XmlRpc::XmlRpcValue list;
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/big_marker_ids", list)) { m_err_param = "big_marker_ids"; return false; }
    ...
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/medium_marker_ids", list)) { m_err_param = "medium_marker_ids"; return false; }
    ROS_ASSERT(list.getType() == XmlRpc::XmlRpcValue::TypeArray);
    for (int32_t i = 0; i < list.size(); ++i) {
        ROS_ASSERT(list[i].getType() == XmlRpc::XmlRpcValue::TypeInt);
        int id = static_cast<int>(list[i]);
    ...
    if (!m_nh.getParam(m_data_ns_param + "/aruco_tracking/small_marker_ids", list)) { m_err_param = "small_marker_ids"; return false; }
    ROS_ASSERT(list.getType() == XmlRpc::XmlRpcValue::TypeArray);
    for (int32_t i = 0; i < list.size(); ++i) {
        ROS_ASSERT(list[i].getType() == XmlRpc::XmlRpcValue::TypeInt);
        int id = static_cast<int>(list[i]);
...
bool Visualizer::InitROS()
{
    ...
    m_setpoint_sub = 
        m_nh.subscribe<kuam_msgs::Setpoint>(m_maneuver_ns_param + "/state_machine/setpoint", 10, boost::bind(&Visualizer::SetpointCallback, this, _1));
    m_state_sub =
        m_nh.subscribe<kuam_msgs::TransReq>(m_maneuver_ns_param + "/state_machine/trans_request", 10, boost::bind(&Visualizer::StateCallback, this, _1));
    m_tasklist_sub = 
        m_nh.subscribe<kuam_msgs::TaskList>(m_maneuver_ns_param + "/mission_manager/tasklist", 10, boost::bind(&Visualizer::TaskListCallback, this, _1));
    ...
    // Initialize timer
    float freq = 5.0;
    m_text_pub_cb_timer = m_nh.createTimer(ros::Duration(1.0/freq), &Visualizer::TextPubCallback, this);
...
void Visualizer::StateCallback(const kuam_msgs::TransReq::ConstPtr &state_ptr)
{
...
{
    // Initialize ROS
	ros::init (argc, argv, "setpoint_visual");
