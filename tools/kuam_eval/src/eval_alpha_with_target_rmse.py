...
class Plotting:
    def __init__(self):
        ...
        self.rmses = [[], [], []]
        self.r_colors = ['red', 'green', 'blue']
        self.r_labels = ['x rmse [m]', 'y rmse [m]', 'z rmse [m]']
        self.r_xlabel = 'time [s]'
        self.r_ylabel = 'rmse [m]'
        self.r_title = 'Error between Marker Ground Truth and Detected Target'

        self.list_set = [self.alpha, self.distances, self.rmses]
        self.color_set = [self.a_colors, self.d_colors, self.r_colors]
        self.label_set = [self.a_labels, self.d_labels, self.r_labels]
        self.xlabel_set = [self.a_xlabel, self.d_xlabel, self.r_xlabel]
        self.ylabel_set = [self.a_ylabel, self.d_ylabel, self.r_ylabel]
        self.title_set = [self.a_title, self.d_title, self.r_title]        
    ...
    def AlphaEvalCB(self, msg):
        if (msg.h_m.data == 0):
            ...
            err_x = GT_TARGET_X - msg.target_pos.x
            err_y = GT_TARGET_Y - msg.target_pos.y
            err_z = GT_TARGET_Z - msg.target_pos.z
